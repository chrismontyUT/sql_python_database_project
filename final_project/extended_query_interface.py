...
def print_menu():
    ...
    print "13. explore twitter with database!!!!!!!"
    print "14. exit"
...
def twitter_menu():
    print "1. See all movies with tweets and how many tweets they have."
    print "2. See each twitter user and how many tweets they posted."
    print "3. See a list of every movie in order of ther user rating and the number of tweets they have."
    print "4. See a list of every movie in order of runtime and the number of tweets they have."
    print "5. See a list of every movie in the database made after 2010 and the number of tweets they have."
...
####### tweet functions below

def num_tweets():

    is_success = True

    query_frame = "select title, COUNT(*) FROM Tweet GROUP BY title"
    ...
    try:
        ...
        is_success = False
        print "tweet query failed: " + e.strerror
    return is_success
...
def twitter_users():
    ...
    query_frame = "select screen_name , COUNT(*)from tweet group by screen_name Order BY COUNT(*) DESC"
...
def tweet_rating():
    ...
    query_frame = "select a.title, a.imdb_user_rating , (select COUNT(*) from Tweet WHERE title = a.title) as tweets from Movie a group by title order by imdb_user_rating"
...
def tweet_runtime():
    ...
    query_frame = "select a.title, a.runtime_in_minutes ,(select COUNT(*) from Tweet WHERE title = a.title) as tweets from Movie a group by title order by tweets desc"
...
def tweet_release_year():

    is_success = True

    query_frame = "select a.title, a.release_year ,(select COUNT(*) from Tweet WHERE title = a.title) as tweets from Movie a WHERE a.release_year > 2010 group by title order by release_year desc"
...
while True:
    ...
    if choice==1:
        ...
        twitter_menu()
        print " "
        new_choice = input("Enter your choice [1-5]: ")
        print " "
        if new_choice == 1:
            print "You have chosen to see all movies and the number of tweets they have."
            print " "
            num_tweets()
        elif new_choice == 2:
            print "You have chosen to see all twitter users and their number of tweets."
            print " "
            twitter_users()
        elif new_choice == 3:
            print "You have chosen to see all movies, thier user rating, and the number of tweets they have."
            print " "
            tweet_rating()
        if new_choice == 4:
            print "You have chosen to see all movies, their runtime in minutes, and the number of tweets they have."
            print " "
            tweet_runtime()
        elif new_choice == 5:
            print "You have chosen to see all movies made after 2010, their release year, and the number of tweets they have."
            print " "
            tweet_release_year()
